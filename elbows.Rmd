...
amoa.cluster_info$first_diffs <- sapply(1:nrow(amoa.cluster_info), first_diffs)
...
```{r}
amoa.cluster_info %>% 
  mutate(first_diffs = first_diffs / max(first_diffs) * max(Abundance)) %>% 
  ggplot(aes(x = reorder(Gene, -Abundance), group = 1)) +
  geom_line(aes(y = cumulative, color = "#F8766D")) +
  geom_line(aes(y = first_diffs, color = "000000")) +
  geom_point(aes(y = cumulative, color = "#F8766D"), size = 0.25) + 
  geom_point(aes(y = first_diffs), size = 0.25) + 
  geom_vline(xintercept = elbow, 
             color = "red",
             linetype = "dashed") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
...
```{r, warning = FALSE}
other_genes_of_interest <- which(amoa.cluster_info$first_diffs > 0.90 * amoa.cluster_info[elbow, 5])
